...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Require at least one approved review row before email_sender runs.",
    )
    p.add_argument(
        "--preflight",
        action="store_true",
...
def _build_website_builder_cmd(node: str, args: argparse.Namespace) -> list[str]:
    cmd = [
        node,
...
def _build_website_builder_env(city: str, categories: list[str]) -> dict[str, str]:
    env = dict(os.environ)
...
def main() -> None:
    ...
    for city in cities:
        ...
        elapsed, step_stats = _run_step(
            name=step_name,
            stage="BUILD",
            cmd=_build_website_builder_cmd(node=node or "node", args=args),
            city=city,
            env=_build_website_builder_env(city, categories),
        )
        timings[step_name] = elapsed
        _merge_stats(pipeline_stats, step_stats)
        _merge_stats(city_stats[city], step_stats)
