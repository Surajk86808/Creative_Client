...
`run_pipeline.py` is the canonical orchestration entrypoint for this monorepo.
...
- `node src/index.js pipeline --city "bengaluru" --categories "salon,gym" --max 0 --analyze-websites --dry-run`
  Deprecated compatibility wrapper that delegates back to repo-root `run_pipeline.py`
