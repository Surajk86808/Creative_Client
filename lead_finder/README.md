...
5. **Sends** personalized emails via Zoho SMTP + Groq LLM
...
lead_finder/
  ...
  email_sender/
    agent.py                  # Main email sender (Groq + SMTP + rate limits + guardrails)
    run.py                    # Alternate CLI sender (category filter, phone-only export)
    guardrails.py             # Email content validation rules
    rate_limiter.py           # Hourly/daily send limits with persistence
    retry_utils.py            # Transient error retry with backoff
    audit_store.py            # JSONL event log + audit trail
    validation.py             # Template file validation
    tests/
...
> **Always run from the project root:** `D:\client finder\lead_finder`
...
You're in the wrong folder. `agent.py` is inside `email_sender/`.
```powershell
cd "D:\client finder\lead_finder"
python .\email_sender\agent.py bengaluru --dry-run
```
...
You ran the sender from the wrong directory. Go back to the project root.
```powershell
cd "D:\client finder\lead_finder"
```
